...
def test_subspace_map_evaluation():
    mesh = df.mesh.create_unit_cube(MPI.COMM_WORLD, 5, 7, 11)
...
def test__identity_map_evaluation() -> None:
    mesh = df.mesh.create_unit_cube(MPI.COMM_WORLD, 5, 7, 11)
    ...
    identity_map, submesh, Q_sub = build_subspace_map(cells, Q_space)
    QV_sub = df.fem.FunctionSpace(submesh, QV)
    QT_sub = df.fem.FunctionSpace(submesh, QT)
    ...
    identity_map.map_to_child(qv, qv_sub)
    identity_map.map_to_child(qt, qt_sub)
    identity_map.map_to_parent(q_sub, q_test)
    identity_map.map_to_parent(qv_sub, qv_test)
    identity_map.map_to_parent(qt_sub, qt_test)
    ...
    qv_view = vector_array.reshape(num_cells, -1)
    qv_test_view = qv_test.x.array.reshape(num_cells, -1)
    assert np.all(qv_view == qv_test_view)

    qt_view = tensor_array.reshape(num_cells, -1)
    qt_test_view = qt_test.x.array.reshape(num_cells, -1)
    assert np.all(qt_view == qt_test_view)
